...
void menu_display() {
  ...
  cout<<"press 3 : for searching the book present  in the library"<<endl;
...
void add_book(node * & kitab) {
  ...
  // if (kitab==NULL) {
  //   //cout<<"in kitab"<<endl;
  //   kitab=new node(number);
  //    kitab->data=number;
  //    kitab->accsession_no=number;
  //   string s1,s2,a1;
  //   int y;
...
void book_details(node * &kitab) {
  ...
  if (show_data(kitab,d1)!=NULL) {
    ...
    cout<<" Accession number is "<<curr->accsession_no<< endl;
...
void pre_order_traversal(node * root) {
  ...
  cout<< root->name_book<<"     " << root->data<<endl;
  if (root->right)
...
void pre_order_traversal1(node * root) {
  ...
  if (root->data!=-1) {
  cout<<root->name<<"   "<<root->data<< "     "<<root->no_of_books_issued<<endl;
  }
  if (root->right)
...
void issue_book(node * mem, node * kitab) {
  ...
  if (show_data(mem,d)!=NULL) {
      node * curr=show_data(mem,d);
      cout<<"Please enter book accession no"<<endl;
      int d1;
      cin>>d1;
      if (show_data(kitab,d1)!=NULL) {
       ...
       if (temp->no_of_copies>0) {
        ...
        cout<<"Successfully you have issued "<<temp->name_book;
        temp->no_of_copies--;
       }
       else {
 ...
 int main() {
  ...
  // }
  // leveltraversal(mem);
  // i read in your ppt  you offer different roles in data intelligence group will i get oppurtunity to work in that different roles.
  // after five years i see myself as an experienced data engineer which is capable of
  // first off all i real likke the work culture of your company and sir i believe that i have the type of knowladge to succeed in your company
  // sir i see this oppurtunity as a way to contribute to an really fast growing company because this company is really fast growing in terms of number of clients, in terms of revenue generated and even in terms of number of employs which are increasing.
